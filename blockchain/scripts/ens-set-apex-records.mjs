...
import { JsonRpcProvider, Wallet, Contract, namehash, isAddress } from 'ethers';
...
for (const a of APEX) {
    ...
    console.log(`\n${a.name} -> ${a.addr}`);
    let tx = await resolver.setAddr(node, a.addr); await tx.wait(); console.log('  addr set:', tx.hash);
