...
| In-app name display | `frontend/js/ens/ens-name.js`; `frontend/js/parcels/ui/parcel-panel.js`; `frontend/js/minted-proposals.js`; `.ens-name-*` in `frontend/css/utilities.css` | Compact ENS line in the parcel info panel (`<slug>.parcels.…`) and on minted-proposal cards (`<id>.proposals.…`): the "ENS ↗" chip links to `app.ens.domains/<name>` (new tab), the name value is click-to-copy. Value ellipsis-truncates (`max-width:100%`) so it never overflows tight 300–400px panels. Helpers mirror the backend slug. |
