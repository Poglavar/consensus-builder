...
--
-- gdi_building_3d stores LOD2 meshes as MultiPolygonZ in EPSG:3765. /buildings/near used to, per request,
...
BEGIN
    LOOP
        WITH batch AS (
            SELECT object_id FROM gdi_building_3d WHERE faces_4326 IS NULL LIMIT 2000
        ),
        faces AS (
            SELECT object_id, (ST_Dump(shape)).geom AS geom
            FROM gdi_building_3d
...
CREATE INDEX IF NOT EXISTS idx_building_3d_geom2d ON gdi_building_3d USING gist (geom2d_3765);

ANALYZE gdi_building_3d;
