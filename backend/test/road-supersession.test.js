...
function proposal(id, roadStatus = 'unapplied', status = 'Active') {
    return { proposalId: id, status, roadProposal: { status: roadStatus } };
}

describe('copied road supersession', () => {
    it('parks an applied source after its combined replacement applies', () => {
        const source = proposal('road-a', 'applied', 'Applied');
        const replacement = { ...proposal('road-b', 'applied', 'Applied'), copiedFromProposalId: 'road-a' };
        ...
        expect(source.roadProposal.status).toBe('unapplied');
    ...
    it('restores the source when the combined road is removed', () => {
        const source = proposal('road-a', 'executed', 'Executed');
        const replacement = { ...proposal('road-b', 'applied', 'Applied'), copiedFromProposalId: 'road-a' };
        const records = new Map([[source.proposalId, source], [replacement.proposalId, replacement]]);
        ...
        expect(source.roadProposal.status).toBe('executed');
    ...
    it('does not park an unapplied source or an unrelated overlapping road', () => {
        ...
        const replacement = proposal('road-b', 'applied', 'Applied');
        const records = new Map([[source.proposalId, source], [replacement.proposalId, replacement]]);
    ...
    it('resolves an applied proposal when its corridor parcel is clicked', () => {
        const road = proposal('road-a', 'applied', 'Applied');
        ...
        road.roadProposal.status = 'unapplied';
        road.status = 'Active';
