...
open data into per-city Postgres tables and serves them to the Three.js view. Two layers so far:
...
flowchart LR
  ...
  N -->|batch UNNEST upsert| Tb[(overture_building)]
  N -->|batch UNNEST upsert| Tt[(overture_tree)]
  Tb -->|ST_DWithin + extrude| Pb[overture-3d provider] --> R[/buildings/near/]
  Tt -->|ST_DWithin| Pt[overture-trees provider] --> D[/decor/near/]
...
| File | Role |
|------|------|
| `buildings/overture-cities.js` | **The generalization point.** One entry per city: ingestion bbox + height-extrusion fallbacks. Also `effectiveHeight()`. Shared by both layers. |
| `buildings/overture-3d.js` | Buildings provider. `near()` → `{ object_id, z_min, z_max, faces[] }` (the common face-mesh shape Zagreb/NYC also yield). |
| `buildings/overture-building-ddl.sql` | `overture_building` table DDL. |
| `decor/overture-trees.js` | Trees provider. `near()` → `{ trees: [[lng,lat], ...] }`. |
| `decor/overture-tree-ddl.sql` | `overture_tree` table DDL (POINT geom). |
| `routes/decor.js` | `POST /decor/near` — scenery (trees now; parks/water later via `kinds`). |
| `scripts/ingest-overture.js` | One layer-parameterized ingestion CLI (`--layer buildings\|trees`). |
| `buildings/index.js` | Wires every `overture-cities.js` key to the buildings provider automatically. |

Frontend: `three-mode.js` renders both. Buildings need only `buildings.source: 'overture'` on the city
config. Trees are an independent instanced layer (two draw calls) with a **"Trees" toggle** in the 3D
panel (persisted in `cb_3d_trees_enabled`, default ON); they reuse the same near-query + radius slider
as the buildings. Tree heights are assigned deterministically per point (Overture has no tree height).
...
- **Restartable**: each load upserts on `(city, overture_id)`, so a re-run refreshes in place.
...
- [x] Generic buildings + trees providers, tables, and one layer-parameterized ingestion CLI.
- [x] Belgrade wired up; both providers verified end-to-end against real Overture data.
- [x] Toggleable instanced trees layer in the 3D view; verified in-browser (render + on/off/on).
- [x] Unit tests (`backend/test/buildings-providers.test.js`).
- [ ] i18n the "Trees" panel label (matches the surrounding hardcoded control labels for now).
- [ ] Productionize ingestion: run on the server / schedule a monthly refresh; local runs only
      populate the local DB.
- [ ] More `kinds` on `/decor/near`: parks/greenery + water polygons (flat ground decals).
