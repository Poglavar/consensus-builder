    ...
    async _applyReparcellizationProposal(proposalId, proposalData, options = {}) {
        ...
        childFeatures.forEach(feature => {
            ...
            if (parcelId !== undefined && parcelId !== null) {
                ...
                // Joint ownership is represented by one deterministic pool agent while the complete
                // member/share ledger remains on the parcel. skipAgentSync keeps the mainline batch
                // rebuild rather than rescanning the full parcel keyspace for every child.
                if (typeof transferParcelOwnership === 'function') {
                    ...
                    if (owners.length > 1) {
                        ...
                        if (ownerKey === 'public-land') {
                            ...
                            agentId = ownerKey;
                        } else if (ownerKey && ownerName && ownerName !== 'Unassigned' && typeof getOrCreateAgentForRecipient === 'function') {
