...
(function attachCadastralTransport(global) {
    ...
    function normalizeFeatureId(feature) {
        if (typeof global.normalizeFeatureParcelId === 'function') return global.normalizeFeatureParcelId(feature);
        const props = feature && feature.properties || {};
        const value = props.parcelId ?? props.parcel_id ?? props.PARCEL_ID ?? props.id;
        if (value === undefined || value === null) return '';
        props.parcelId = String(value);
    ...
    function datasetToLatLng(easting, northing) {
        ...
        if (manager && typeof manager.datasetToLatLng === 'function') {
            const result = manager.datasetToLatLng(easting, northing);
    ...
    async function fetchCell(cell) {
        const gridSize = Number(global.ParcelsState?.getParcelGridSize?.()
        ...
        const sw = datasetToLatLng(swE, swN);
        const ne = datasetToLatLng(neE, neN);
        ...
        const features = [];
        while (more) {
            const request = builder ? builder(bbox, { count, startIndex, latLonBbox }) : null;
    ...
    async function fetchBounds(_bounds, options = {}) {
        ...
        for (const batch of chunks(keys, 6)) {
            const part = await Promise.all(batch.map(fetchCell));
            results.push(...part);
            done += batch.length;
    ...
    global.__cadastralGroundTransport = Object.freeze({ fetchByIds, fetchBounds, fetchUnderGeometry });
